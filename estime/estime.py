...
class Estime:
    ...
    To get additional measures - range-restricted coherence measures - include 
    the ranges you are interested in, by providing coherence_ranges_min_max.
    """
    def __init__(
        self,
    ...
    def _evaluate(self, embs_summ, embs_text, embs_summ_raw, embs_text_raw):
        ...
        Returns:
            List[float)] - List of results as specified in self.output, followed
                by the range-restricted coherence values 
                (as many values as the length of self.ranges_min_max)
        """
    ...
    def get_kendall_range(self, itoks_pairs, range_min=-1, range_max=1000000000):
        """
        Kendall Tau correlation, with count of the pairs restricted by the distance in the first pair.
        itoks_pairs is a sequence of tuples, each tuple must have two first elements, 
            of which the correlaiton is calculated here.
        """
        nc, nd, n = 0, 0, 0  # concord, discord and total pairs
        for pair1 in itoks_pairs:
            for pair2 in itoks_pairs:
                if pair1[0] == pair2[0] or abs(pair1[0] - pair2[0]) < range_min or abs(pair1[0] - pair2[0]) > range_max:
                    continue
                n += 1
